...
async function executeAutofix(job: PipelineJob, config: DaemonConfig): Promise<void> {
  ...
  if (issue.repoCloneUrl.startsWith("https://github.com") || issue.repoCloneUrl.startsWith("git@github.com")) {
    ...
    const askpassPath = writeAskpassScript(config.githubToken, tempDir);

    const cleanUrl = issue.repoCloneUrl;
    const askpassEnv = getAskpassEnv(askpassPath);

    await notifyStatus(job, "cloning", `Cloning ${issue.repoFullName}...`);

    const cloneResult = await executeTool("run_bash", {
      command: `${askpassEnv} git clone --depth=1 ${cleanUrl} "${cloneDir}" 2>&1`,
      timeout: 120_000,
    });
    if (!cloneResult.success) { cleanAskpass(askpassPath); throw new Error(`Clone failed: ${cloneResult.error}`); }
...
Generated by MTC autonomous bug-fixing pipeline." 2>&1`,
    ...
    const pushResult = await executeTool("run_bash", {
      command: `cd "${cloneDir}" && ${getAskpassEnv(askpassPath)} git push "${cleanUrl}" "${branchName}" 2>&1`,
      timeout: 60_000,
    });
    cleanAskpass(askpassPath);
...
function writeAskpassScript(token: string, dir: string): string {
  const isWin = process.platform === "win32";
  const ext = isWin ? ".bat" : ".sh";
  const path = join(dir, "git-askpass" + ext);
  const content = isWin
    ? `@echo x-access-token\r\n@echo ${token}\r\n`
    : `#!/bin/sh\necho "x-access-token"\necho "${token}"\n`;
...
function cleanAskpass(path: string): void {
  try { unlinkSync(path); } catch {}
}

function getAskpassEnv(askpassPath: string): string {
  const isWin = process.platform === "win32";
  return isWin ? `set "GIT_ASKPASS=${askpassPath}" &&` : `GIT_ASKPASS="${askpassPath}"`;
}
